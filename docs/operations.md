...
| R2 CORS | Bucket CORS rule so the browser's presigned upload isn't blocked | 4.11 | ☐ set 2026-09-18 without the custom domain — uploads from agnte.app still blocked |
...
written and merged, so it read as "still to build" when it meant "written,
...
```bash
# Most of Phase 1 and 4 in one answer — email, signing key, Google, storage, db
curl -s https://agnte.app/v1/health | jq '.checks'

gcloud tasks queues describe agnte-media-thumbnails --location=europe-west3 --project=agnte-prod
gcloud scheduler jobs list --location=europe-west3 --project=agnte-prod
gcloud run jobs list --region=europe-west3 --project=agnte-prod
```

`/v1/health` is the closest thing to an authority here, and it has a gap worth
knowing: it reports the database, object storage, email, access tokens and
Google sign-in, but says nothing about Cloud Tasks — which is precisely the
one that broke. Adding it would make the whole class of "configured but not
really" visible in one request.
...
### When pg_dump starts refusing: the client must lead the server

`pg_dump` will not dump a server newer than itself. Neon upgrades its Postgres
major without asking, so the pin in `infra/backup/Dockerfile` — `ARG PG_MAJOR`
— eventually trails the server and every run dies with:
...
Bump `PG_MAJOR` to at least the server's major and redeploy. This has happened
once already, when Neon moved to 18 and the pin was still 17; `PG_MAJOR=18`
against Neon's 18.6 is confirmed working, and `postgresql-client-18` does
resolve from `bookworm-pgdg`.

Note that the production deploy workflow does **not** build this image — it
only builds the web service. A change to `infra/backup/` reaches production
only when someone runs `deploy-backup-job.sh`, which is its own way for a fix
to look shipped and not be.

The pin stays deliberate — an unannounced client upgrade is its own hazard —
...
non-2xx as a retry, not an alert; the status table said ☐ and was assumed to
mean "not set up yet" rather than "running nightly and failing"; `/v1/health`
reports the database and R2 but says nothing about whether anything was ever
...
Anything other than `Succeeded` in the status column means there is no backup
from that night.
...
That is not hypothetical — it is how this row came to be ticked and uploads
still broken. The script now says so out loud when it finds no domain.
